...
export default function Categories() {
    ...
    const deleteCategory = element => {
        if (element.numDocs > 0) return;
        if (categories.some(c => c.parent === element.id)) return;

        if (!element.temp) {
            rq(`/categories/${element.id}`, {
                method: 'DELETE'
            }).then(res => {
                if (res.ok) return;
                ...
                addNewCategoryToList(element.parent, true, element);
            });
        }
